	...
	?>
		...
		<form name="list" id="list" class="form-horizontal" method="post" action="vqp_check_save.php"> 
			...
			?>
					...
					<label class="control-label" for="inputEmail"></label>
					<div class="controls">
						<input name="vqp_nname" id="vqp_nname" pattern="^[A-z][a-zA-Z\s]*$" maxlength="20" type="hidden" placeholder="Full Name" value="<?php echo $vqp_nname ?>" required />
					</div>
					</div>
...
if($data[0] < 1)
{
  ...
  mysqli_query($link,"INSERT INTO vqpusers (vqp_code,vqp_password,vqp_name,vqp_nname,vqp_store,vqp_position,vqp_status,vqp_date,rDate) values('$vqp_code','$vqp_password','$vqp_name','$vqp_nname','$vqp_store','$vqp_position','$vqp_status','$vqp_date','$rdate')")or die(mysqli_error());
